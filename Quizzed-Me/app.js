...
function endQuiz() {
    if(myAnswers[lengthofobject-1]) {
        var output = "<div class='output'>Quiz Results"
        ...
        for( var i = 0; i< myAnswers.length; i++) {
            if(data.quizcontent[i].correct == myAnswers[i]) {
                ...
                console.log(temp1)
                temp='<p> the tagline of '+ data.quizcontent[i].question + ' is :'+  data.quizcontent[i]["a"+data.quizcontent[i].correct]+'</p>'
                output = output + '<p>Question '+ (i+1) + ' ' +questionResult+ temp+'</p> <br>';
